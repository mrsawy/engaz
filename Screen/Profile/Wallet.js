...
import { StyleSheet, Text,Image, View ,ScrollView ,Linking,AsyncStorage,ActivityIndicator,TouchableOpacity,Platform,StatusBar,FlatList,RefreshControl} from 'react-native';
...
import RBSheet from "react-native-raw-bottom-sheet";
 ...
 class Wallet extends React.Component {
      ...
      RenderData(){
        ...
        if(this.props.data){
          return(
             <Text style={{textAlign:'center',fontFamily: 'Medium',fontSize:30,color:'#454545',marginTop:4}}>{this.props.data.available_balance} {i18n.t('riyal')}</Text>
          )
        }
  
        if(!this.props.data){
          return(
            <Text style={{textAlign:'center',fontFamily: 'Medium',fontSize:30,color:'#454545',marginTop:4}}>0 ريال</Text>
          )
        }
      }
      ...
      RenderDataLog(){
        ...
        if(this.props.data_log){
          return(
            ...
            renderItem={({item}) => 
              <View style={{padding:15,flexDirection:this.props.Language == 'ar' ?'row-reverse':'row',marginLeft:-15,marginRight:-15,borderBottomWidth:1,borderBottomColor:'#eee'}}>
                  <Text style={{lineHeight:25,flex:1,textAlign:this.props.Language == 'ar' ?'right':'left',fontFamily: 'Medium',fontSize:15,color:'#454545',marginTop:4}}>{item.description}</Text>
                  <Text style={{width:80,textAlign:this.props.Language == 'ar' ? 'left':'right',fontFamily: 'BOLD',fontSize:13,color:'#454545',marginTop:4}}>{item.movement_type == 'deposit' ? '+' : '-'} {item.value} {i18n.t('riyal')}</Text>
              </View>
            } 
          ...
          <View style={styles.container}>
           <StatusBar translucent={true}  barStyle="light-content" backgroundColor={'#2A2A2A'} />
            ...
            <View style={{justifyContent:'center',flex:1,}}>
              <Text style={{textAlign:'center',fontFamily: 'Medium',fontSize:15,color:'#fff',marginTop:2}}>{i18n.t('my_wallet')}</Text>
            </View>
          ...
          <TouchableOpacity onPress={()=> this.props.navigation.navigate('Messages')} style={{width:35,marginLeft:5}}>
            ...
            {/* <View style={{position:'absolute',left:17 ,justifyContent:'center',
                ...
                <View style={{margin:15}}>
                    ...
                    <View style={{padding:30,backgroundColor:'#eee',borderWidth:1,borderColor:'#ccc',borderRadius:10}}>
                        {this.RenderData()}
                    </View>

                    <View style={{padding:15,backgroundColor:'#454545',borderRadius:10,marginTop:15,marginBottom:30}}>
                        <Text style={{textAlign:'center',fontFamily: 'Medium',fontSize:15,color:'#fff',marginTop:4}}>{i18n.t('log_wallet')}</Text>
                    </View>
...
export default connect(mapStateToProps, {GetWallet,GetWalletLog})(Wallet);