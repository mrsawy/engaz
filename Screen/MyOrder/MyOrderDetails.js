...
class MyOrderDetails extends React.Component {
  ...
  EditOffer(id) {
    this.setState({ loading_approved: true });

    fetch(API_URI + "/api/edit_offer", {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
      ...
      body: JSON.stringify({
        id_order: this.props.route.params.item.id,
        price: this.props.route.params.item.request_edit_price,
      }),
    })
      .then((response) => response.json())
      .then(async (responseJson) => {
        this.setState({ loading_approved: false });

        if (responseJson.status == "true") {
          showMessage({
            message: i18n.t("edit"),
            description: i18n.t("edit_s"),
            type: "success",
            backgroundColor: "green",
          });
  ...
  DisEditOffer(id) {
    this.setState({ loading_disapproved: true });

    fetch(API_URI + "/api/dis_edit_offer", {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
      ...
      .then((response) => response.json())
      .then(async (responseJson) => {
        this.setState({ loading_disapproved: false });

        if (responseJson.status == "true") {
          showMessage({
            message: i18n.t("edit_false"),
            description: i18n.t("edit_s_false"),
            type: "success",
            backgroundColor: "green",
          });
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        >
          {this.props.route.params.item.request_edit_price ? (
            <View style={{ backgroundColor: "#F97534", padding: 15 }}>
              <View
                style={{
              ...
              >
                ...
                >
                  {i18n.t("edit_msg_des_pages")}{" "}
                  {this.props.route.params.item.request_edit_price}{" "}
                  {i18n.t("riyal")}
                </Text>

                {!this.state.loading_approved ? (
                  <TouchableOpacity
                    onPress={() =>
                      this.EditOffer(this.props.route.params.item.id)
                    }
                    style={[
                      {
                        width: 70,
                ...
                {!this.state.loading_disapproved ? (
                  <TouchableOpacity
                    onPress={() =>
                      this.DisEditOffer(this.props.route.params.item.id)
                    }
                    style={[
                      {
                        width: 70,
          ...
          <View style={{ margin: 15 }}>
            ...
            >
                ...
                {this.props.route.params.item.order_status == "new" ? (
                  <Menu style={{ width: 30, height: 30 }}>
                    ...
                    <MenuOptions>
                      <MenuOption
                        onSelect={() =>
                          this.Cancel(this.props.route.params.item.id)
                        }
                        style={{
                          padding: 10,
                          flexDirection:
              ...
              {!this.props.route.params.item.user_id_offers ? (
                <View
                  style={{
                    flex: 1,
                    flexDirection:
                ...
                >
                  ...
                  >
                    ...
                    >
                      <Image
                        source={
                          this.props.route.params.item.user_id_offers.photo
                            ? {
                                uri:
                                  API_URI +
                                  "/storage/" +
                    ...
                    >
                      ...
                      >
                        {this.props.route.params.item.user_id_offers.name}
                      </Text>
                      <View
                        style={{
                          flexDirection:
                      ...
                      >
                        <StarRating
                          ...
                          maxStars={5}
                          rating={Number(
                            this.props.route.params.item.user_id_offers.rating
                          ).toFixed(2)}
                          reversed
                    ...
                    <TouchableOpacity
                      onPress={() =>
                        this.SendMsg(
                          this.props.route.params.item.user_id_offers.id,
            ...
            {this.state.page == "Details" ? (
              <ScrollView>
                <View style={{ flex: 1, marginTop: 20 }}>
                  ...
                  {this.props.route.params.item.order_status == "in_progress" &&
                  this.props.route.params.item.request_finished == "yes" ? (
                    <Text
                      style={{
                        flex: 1,
                        fontSize: 14,
                    ...
                    >
                      {i18n.t("request_finished_orders_des")}
                    </Text>
                  ) : null}
                  ...
                  "in_progress" ? (
                    <View>
                      {!this.props.loading_complete ? (
                        <TouchableOpacity
                          onPress={() =>
                            this.props.navigation.navigate("RatingService", {
                              id: this.props.route.params.item.id,
                            })
                          }
                          style={[
